...
class TlsConfigSourceCertificateGenerationTest {
    ...
    @Test
    void legacyCaEndpointServesRootInsteadOfServerLeaf() throws Exception {
        new TlsConfigSource();
        EmulatorConfig config = mock(EmulatorConfig.class, RETURNS_DEEP_STUBS);
        when(config.tls().enabled()).thenReturn(true);
        when(config.storage().persistentPath()).thenReturn(tempDir.toString());

        try (Response response = new TlsCaController(config).getCaCertificate()) {
            assertEquals(200, response.getStatus());
            assertEquals(Files.readString(tempDir.resolve("tls/floci-root-ca.crt")), response.getEntity());
            assertNotEquals(Files.readString(tempDir.resolve("tls/floci-server.crt")), response.getEntity());
        }
    }
    ...
    void testCertificateWithDefaultConfiguration() throws Exception {
        ...
        assertTrue(sans.contains("*.execute-api.us-east-1.amazonaws.com"),
            "Certificate SANs should include API Gateway execute-api host *.execute-api.us-east-1.amazonaws.com");
        assertTrue(sans.contains("*.lambda-microvm.us-east-1.localhost.floci.io"),
            "Certificate SANs should include Lambda MicroVM endpoints");
