...
class AssumeRoleWithSamlValidationIntegrationTest {
    ...
    @Test
    void providerExtensionsPreserveTheMetadataUsedBySts() {
        String role = createRole(true);
        given().contentType("application/x-www-form-urlencoded")
                .formParam("Action", "TagSAMLProvider").formParam("SAMLProviderArn", PROVIDER)
                .formParam("Tags.member.1.Key", "merge-owner").formParam("Tags.member.1.Value", "combined")
                .header("Authorization", auth("iam")).when().post("/").then().statusCode(200);
        given().contentType("application/x-www-form-urlencoded")
                .formParam("Action", "UpdateSAMLProvider").formParam("SAMLProviderArn", PROVIDER)
                .formParam("AssertionEncryptionMode", "Allowed")
                .header("Authorization", auth("iam")).when().post("/").then().statusCode(200);
        given().contentType("application/x-www-form-urlencoded")
                .formParam("Action", "GetSAMLProvider").formParam("SAMLProviderArn", PROVIDER)
                .header("Authorization", auth("iam")).when().post("/").then().statusCode(200)
                .body(containsString("combined"))
                .body(containsString(certificateBase64));
        assume(role, assertion(role, PROVIDER, Instant.now().plusSeconds(300), AUDIENCE, ISSUER, true), PROVIDER)
                .statusCode(200)
                .body("AssumeRoleWithSAMLResponse.AssumeRoleWithSAMLResult.Credentials.AccessKeyId", startsWith("ASIA"));
    }
