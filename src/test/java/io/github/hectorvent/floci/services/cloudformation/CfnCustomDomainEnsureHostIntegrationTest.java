 ...
 * new name is registered too; an in-place update and the stack delete register nothing. The
 * stack starts with the certificate alone, which is DNS-validated before the domains use it.
 */
@QuarkusTest
class CfnCustomDomainEnsureHostIntegrationTest {
    ...
    /** The certificate alone: the custom domains need it ISSUED, so it is DNS-validated first. */
    private static final String CERTIFICATE_TEMPLATE = """
        {
          "Resources": {
            "Cert": {
              "Type": "AWS::CertificateManager::Certificate",
              "Properties": {"DomainName": "*%s", "ValidationMethod": "DNS"}
            }
          },
          "Outputs": {
    ...
    void stackRegistersEveryCustomDomainAndAReplacementButNotAnInPlaceUpdateOrTheDelete() throws Exception {
        cloudFormation("CreateStack", CERTIFICATE_TEMPLATE, Map.of());
        String certificateArn = XmlParser.extractPairs(awaitStackStatus("CREATE_COMPLETE"),
                "Outputs", "OutputKey", "OutputValue").get("CertArn");
        AcmDnsValidation.validate(null, certificateArn);

        cloudFormation("UpdateStack", Map.of("ApiDomainName", API_DOMAIN));
    ...
    private static void cloudFormation(String action, Map<String, String> parameters) {
        cloudFormation(action, TEMPLATE, parameters);
    }

    private static void cloudFormation(String action, String template, Map<String, String> parameters) {
    ...
    private static String awaitStackStatus(String expected) throws InterruptedException {
        String body = "";
        for (int i = 0; i < 200; i++) {
            body = describeStack();
