...
@QuarkusTest
class AppFlowFlowIntegrationTest {
    ...
    void createFlowWithAnEmptyS3PrefixFailsWithConnectorServerException() {
        ...
        given()
                .contentType("application/json")
                .header("Authorization", auth(EAST))
                .body(flowBody("empty-prefix-flow", bucket, "input"))
                .when()
                .post("/create-flow")
                .then()
                .statusCode(400)
    ...
    void createDescribeStartStopCancelDeleteAndTagsLifecycle() {
        ...
        String flowName = "lifecycle-flow-" + UUID.randomUUID().toString().substring(0, 8);
        s3Service.createBucket(bucket, EAST);
        ...
        String arn = given()
                ...
                .body(flowBody(flowName, bucket, "input"))
                .when()
                .post("/create-flow")
                .then()
                .statusCode(200)
    ...
    private static String flowBody(String flowName, String bucket, String prefix) {
        return """
                {
                  "flowName":"%s",
                ...
                """.formatted(flowName, bucket, prefix, bucket);
    }
