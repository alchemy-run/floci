...
class IamServiceTest {
    ...
    @Test
    void forkRoleSessionRetainsTheTokenRequiredByUpstreamAuthentication() {
        IamService.RoleSessionCredentials credentials = iamService.mintRoleSession(
                "arn:aws:iam::000000000000:role/execution-role");

        assertEquals(credentials.sessionToken(), iamService.findSessionToken(credentials.accessKeyId()).orElseThrow());
        assertEquals(credentials.secretAccessKey(),
                iamService.findSecretKey(credentials.accessKeyId(), credentials.sessionToken()).orElseThrow());
        assertTrue(iamService.findSecretKey(credentials.accessKeyId(), "wrong-token").isEmpty());
        assertTrue(iamService.isAssumedRoleSession(credentials.accessKeyId()));
    }
