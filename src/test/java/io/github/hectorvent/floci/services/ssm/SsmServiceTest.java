...
class SsmServiceTest {
    ...
    @Test
    void describeParametersFiltersStoredForkMetadata() {
        String region = "us-east-1";
        ssmService.putParameter("/custom-key", "value", "SecureString", null, false, region,
                Map.of(), "Advanced", "alias/custom", null, "text");

        assertEquals(List.of("/custom-key"),
                describedNames(List.of(filter("Tier", null, "Advanced")), region));
        assertEquals(List.of("/custom-key"),
                describedNames(List.of(filter("KeyId", null, "alias/custom")), region));
        assertTrue(describedNames(List.of(filter("KeyId", null, "alias/aws/ssm")), region).isEmpty());
    }
    ...
    void putParameterWithTagsPersistsTags() {
        ...
        Parameter described = ssmService.describeParameters(List.of(filter("Name", "Equals", "/app/tagged")), region).getFirst();
