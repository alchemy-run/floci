...
class CloudWatchLogsMetricFilterStorageReloadTest {
    ...
    void legacyForkFilterKeysMigrateWithoutLosingTransformationsOrAccountIsolation(String mode) {
        String legacyKey = REGION + "::" + GROUP + "::filter::legacy";
        try (Services first = open(mode, ACCOUNT)) {
            first.group(GROUP, REGION);
            MetricFilter filter = definition("legacy", "ERROR", "1", 0.0);
            filter.setCreationTime(1234L);
            first.logs.metricFilterStore().put(legacyKey, filter);
        }
        try (Services other = open(mode, OTHER_ACCOUNT)) {
            assertEquals(0, other.filters.countMetricFilters(GROUP, REGION));
        }
        try (Services reloaded = open(mode, ACCOUNT)) {
            MetricFilter filter = reloaded.filters.findMetricFilter(GROUP, "legacy", REGION).orElseThrow();
            assertEquals(1234L, filter.getCreationTime());
            assertEquals(0.0, filter.getMetricTransformations().getFirst().getDefaultValue());
            assertTrue(reloaded.logs.metricFilterStore().get(legacyKey).isEmpty());
            assertEquals(1, reloaded.filters.countMetricFilters(GROUP, REGION));
            reloaded.filters.deleteMetricFilter(GROUP, "legacy", REGION);
            assertEquals(0, reloaded.filters.countMetricFilters(GROUP, REGION));
        }
    }
