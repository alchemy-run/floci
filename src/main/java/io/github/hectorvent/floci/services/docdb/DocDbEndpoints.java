...
/**
 * DocumentDB endpoint hostnames in the shape AWS gives them, with Floci's DNS suffix in place of
 * {@code amazonaws.com}.
 *
 * <p>AWS names a cluster {@code <id>.cluster-<hash>.<region>.docdb.amazonaws.com}, its reader
 * {@code <id>.cluster-ro-<hash>...} and an instance {@code <id>.<hash>.<region>.docdb...}, where
 * the 12-character hash is fixed per account and region. The suffix is one Floci's DNS already
 * answers: {@code *.localhost.floci.io} is public wildcard DNS for 127.0.0.1 on the host, and
 * Floci's embedded DNS resolves it to Floci inside the containers it launches.
 */
final class DocDbEndpoints {

    private static final String ALPHABET = "abcdefghijklmnopqrstuvwxyz0123456789";
    private static final int HASH_LENGTH = 12;

    private DocDbEndpoints() {
    }
    ...
    private static String host(String id, String label, String region, String suffix) {
        return id.toLowerCase(Locale.ROOT) + "." + label + "." + region + ".docdb." + suffix;
    }
}
