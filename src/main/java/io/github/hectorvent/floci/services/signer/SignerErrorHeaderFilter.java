 ...
 * the body; stamp the header on Signer error responses.
 */
@Provider
@Priority(1)
public class SignerErrorHeaderFilter implements ContainerResponseFilter {

    @Override
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        String path = request.getUriInfo().getPath();
        if (path == null || !(path.contains("signing-profiles")
                || path.contains("signing-jobs")
                || path.contains("signing-platforms")
                || path.contains("revocations")
                || path.contains("aws-signer"))) {
            return;
        }
