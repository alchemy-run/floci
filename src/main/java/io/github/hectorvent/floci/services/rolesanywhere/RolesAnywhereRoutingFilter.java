...
/**
 * Roles Anywhere restJson1 paths such as {@code /profiles} and
 * {@code /trustanchor/{id}} collide with S3's path-style catch-all. Floci
 * serves every service on one port, so SigV4 credential scope
 * {@code rolesanywhere} is rewritten onto an internal prefix the controller
 * owns.
 */
@Provider
@PreMatching
@Priority(10)
public class RolesAnywhereRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-rolesanywhere";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isRolesAnywhere(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isRolesAnywhere(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && RolesAnywhereService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }
