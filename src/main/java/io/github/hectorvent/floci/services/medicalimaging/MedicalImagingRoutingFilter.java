...
/**
 * HealthImaging restJson1 paths such as {@code /datastore} collide with S3's
 * path-style {@code /{bucket}} catch-all. Floci serves every service on one
 * port, so SigV4 credential scope {@code medical-imaging} is rewritten onto an
 * internal prefix the Medical Imaging controller owns. Tag APIs stay on
 * {@code /tags/{arn}} and are dispatched by {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class MedicalImagingRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/medical-imaging";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isMedicalImaging(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isMedicalImaging(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && MedicalImagingService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }
