...
/**
 * DSQL restJson1 paths such as {@code /cluster} and {@code /stream/{id}} collide with
 * S3's path-style catch-all. Floci serves every service on one port, so SigV4
 * credential scope {@code dsql} is rewritten onto an internal prefix the DSQL
...
@PreMatching
@Priority(10)
public class DsqlRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-dsql";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isDsql(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isDsql(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && DsqlService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }

    /**
     * Prefixes dsql-signed paths onto {@link #INTERNAL_PREFIX}, stripping a trailing
