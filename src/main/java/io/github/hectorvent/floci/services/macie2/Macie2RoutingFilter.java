...
/**
 * Amazon Macie restJson1 shares short paths ({@code /macie}, {@code /admin},
 * {@code /members}) with other services and S3's {@code /{bucket}} catch-all.
 * SigV4 credential scope {@code macie2} is rewritten onto an internal prefix
 * the Macie2 controller owns. Tag APIs stay on {@code /tags/{arn}}.
 */
@Provider
@PreMatching
@Priority(10)
public class Macie2RoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/macie2";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isMacie2(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isMacie2(String authorization) {
