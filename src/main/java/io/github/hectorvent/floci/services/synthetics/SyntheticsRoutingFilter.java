...
/**
 * CloudWatch Synthetics restJson1 shares short paths ({@code /canary},
 * {@code /group}) with S3's {@code /{bucket}} catch-all. SigV4 credential
 * scope {@code synthetics} is rewritten onto an internal prefix the
 * Synthetics controller owns. Tag APIs stay on {@code /tags/{arn}}.
 *
 * <p>Function URL invocations are rewritten to {@code /lambda-url/{urlId}/...}
 * before this filter. Prefixing those paths with {@code /synthetics} 404s the
 * Lambda fixture the Bindings suite probes at {@code /bindings} and
 * {@code /canary}.
 */
@Provider
@PreMatching
@Priority(10)
public class SyntheticsRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/synthetics";
    static final String SERVICE = SyntheticsService.SERVICE;
    ...
    public void filter(ContainerRequestContext requestContext) {
        ...
        if (!isSynthetics(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isSynthetics(String authorization) {
