 ...
 * the body; stamp the header on notifications-contacts error responses.
 */
@Provider
@Priority(1)
public class NotificationsContactsErrorHeaderFilter implements ContainerResponseFilter {

    @Override
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        String path = request.getUriInfo().getPath();
        if (path == null || !path.contains("emailcontacts")) {
            return;
        }
