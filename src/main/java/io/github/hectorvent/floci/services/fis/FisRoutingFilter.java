...
/**
 * AWS FIS restJson1 paths such as {@code /experiments} and {@code /actions/{id}}
 * collide with S3's path-style {@code /{bucket}} and {@code /{bucket}/{key}}
 * catch-alls. Floci serves every service on one port, so SigV4 credential
 * scope {@code fis} is rewritten onto an internal prefix the FIS controller
 * owns. Tag APIs stay on {@code /tags/{arn}} and are dispatched by
 * {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class FisRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-fis";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isFis(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isFis(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && FisService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }
