 ...
 * the body; stamp the header on RUM app-monitor error responses.
 */
@Provider
@Priority(1)
public class RumErrorHeaderFilter implements ContainerResponseFilter {

    @Override
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        String path = request.getUriInfo().getPath();
        if (path == null || !(path.contains("appmonitor") || path.contains("rummetrics"))) {
            return;
        }
