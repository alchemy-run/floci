 ...
 * the body; stamp the header on EventBridge Schema Registry error responses.
 */
@Provider
@Priority(1)
public class SchemasErrorHeaderFilter implements ContainerResponseFilter {

    @Override
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        String path = request.getUriInfo().getPath();
        if (path == null || !(path.contains("v1/registries")
                || path.contains("v1/policy")
                || path.contains("v1/discover"))) {
            return;
        }
