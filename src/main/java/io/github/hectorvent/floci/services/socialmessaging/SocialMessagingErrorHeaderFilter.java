 ...
 * the body; stamp the header on Social Messaging error responses.
 */
@Provider
@Priority(1)
public class SocialMessagingErrorHeaderFilter implements ContainerResponseFilter {

    @Override
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        String path = request.getUriInfo().getPath();
        if (path == null || !(path.contains("social-messaging") || path.contains("whatsapp")
                || path.contains("aws-social-messaging"))) {
            return;
        }
