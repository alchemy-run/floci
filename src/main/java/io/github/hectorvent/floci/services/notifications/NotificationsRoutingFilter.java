...
/**
 * User Notifications restJson1 paths such as {@code /notification-configurations}
 * and {@code /channels} collide with IoT Managed Integrations and DevOps Guru.
 * Requests signed for {@code notifications} are rewritten onto an internal prefix
...
@PreMatching
@Priority(10)
public class NotificationsRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-notifications";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isNotifications(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isNotifications(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && NotificationsService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }
