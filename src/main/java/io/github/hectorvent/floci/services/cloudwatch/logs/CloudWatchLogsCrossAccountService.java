...
public class CloudWatchLogsCrossAccountService {
    ...
    public List<LogDestination> describeDestinations(String prefix, String region) {
        String keyPrefix = region + "::destination::";
        return destinations.scan(key -> key.startsWith(keyPrefix)).stream()
                .filter(destination -> prefix == null || destination.getDestinationName().startsWith(prefix))
                .sorted(Comparator.comparing(LogDestination::getDestinationName))
                .toList();
    }

    public void deleteDestination(String name, String region) {
        String key = destinationKey(region, name);
        if (destinations.get(key).isEmpty()) {
            throw new AwsException("ResourceNotFoundException", "The specified destination does not exist: " + name, 400);
        }
        destinations.delete(key);
    }

    private static String destinationKey(String region, String name) { return region + "::destination::" + name; }
