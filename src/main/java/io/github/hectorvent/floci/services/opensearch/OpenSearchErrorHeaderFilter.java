 ...
 * the body; stamp the header for OpenSearch ({@code /2021-01-01}) paths.
 */
@Provider
@Priority(1)
public class OpenSearchErrorHeaderFilter implements ContainerResponseFilter {

    @Override
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        String path = request.getUriInfo().getPath();
        if (path == null || !path.contains("2021-01-01")) {
            return;
        }
