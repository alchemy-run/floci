...
/**
 * VPC Lattice restJson1 paths such as {@code /services} collide with Audit Manager
 * and Application Signals. Floci serves every service on one port, so SigV4
 * credential scope {@code vpc-lattice} is rewritten onto an internal prefix the
 * VPC Lattice controller owns. Tag APIs stay on {@code /tags/{arn}} and are
 * dispatched by {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class VpcLatticeRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-vpclattice";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isVpcLattice(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isVpcLattice(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && VpcLatticeService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }

    /**
     * Prefixes vpc-lattice-signed paths onto {@link #INTERNAL_PREFIX}.
