...
/**
 * EMR Serverless restJson1 paths such as {@code /applications} collide with
 * S3's path-style catch-all. Floci serves every service on one port, so SigV4
 * credential scope {@code emr-serverless} is rewritten onto an internal prefix
 * the controller owns. Tag APIs stay on {@code /tags/{arn}} and are dispatched
 * by {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class EmrServerlessRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-emr-serverless";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isEmrServerless(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isEmrServerless(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && EmrServerlessService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }

    /**
     * Prefixes emr-serverless-signed paths onto {@link #INTERNAL_PREFIX}, stripping a
