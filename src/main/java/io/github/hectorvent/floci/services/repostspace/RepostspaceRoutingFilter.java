...
/**
 * AWS re:Post Private restJson1 paths such as {@code /spaces/{spaceId}/channels}
 * collide with S3's {@code /{bucket}/{key}} catch-all. SigV4 credential scope
 * {@code repostspace} is rewritten onto {@link #INTERNAL_PREFIX} so this
 * controller owns those routes. Tag APIs stay on {@code /tags/{arn}}.
 */
@Provider
@PreMatching
@Priority(10)
public class RepostspaceRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-repostspace";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isRepostspace(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isRepostspace(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && RepostspaceService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }
