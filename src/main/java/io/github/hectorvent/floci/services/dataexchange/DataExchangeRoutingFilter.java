...
/**
 * Data Exchange restJson1 paths such as {@code /v1/data-sets} collide with S3's
 * path-style {@code /{bucket}/{key:.+}} catch-all. Floci serves every service on
 * one port, so SigV4 credential scope {@code dataexchange} is rewritten onto an
 * internal prefix the Data Exchange controller owns. Tag APIs stay on
 * {@code /tags/{arn}} and are dispatched by {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class DataExchangeRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-dataexchange";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isDataExchange(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isDataExchange(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && DataExchangeService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }

    /**
     * Prefixes dataexchange-signed paths onto {@link #INTERNAL_PREFIX}. {@code /tags}
