...
/**
 * DataZone restJson1 paths such as {@code /v2/domains} collide with S3's path-style
 * {@code /{bucket}/{key:.+}} catch-all. Floci serves every service on one port, so
 * SigV4 credential scope {@code datazone} is rewritten onto an internal prefix the
 * DataZone controller owns. Tag APIs stay on {@code /tags/{arn}} and are dispatched
 * by {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class DataZoneRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-datazone";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isDataZone(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isDataZone(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && DataZoneService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }

    /**
     * Prefixes datazone-signed paths onto {@link #INTERNAL_PREFIX}. {@code /tags}
