 ...
 * the body {@code __type}. Stamp the header on Resource Explorer error
 * responses.
 */
@Provider
@Priority(1)
public class ResourceExplorerErrorHeaderFilter implements ContainerResponseFilter {

    @Override
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        String path = request.getUriInfo().getPath();
        if (path == null || !path.contains("resource-explorer-2")) {
            return;
        }
