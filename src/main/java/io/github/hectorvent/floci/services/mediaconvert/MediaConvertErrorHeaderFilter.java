 ...
 * the body; stamp the header for MediaConvert paths.
 */
@Provider
@Priority(1)
public class MediaConvertErrorHeaderFilter implements ContainerResponseFilter {

    @Override
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        String path = request.getUriInfo().getPath();
        if (path == null || !path.contains("2017-08-29")) {
            return;
        }
