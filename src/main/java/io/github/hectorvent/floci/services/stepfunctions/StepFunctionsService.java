...
public class StepFunctionsService implements Resettable, ResourceProvider {
    ...
    public StateMachine updateStateMachine(String arn, String definition, String roleArn,
        ...
        return updateStateMachine(arn, new UpdateStateMachineRequest(definition, roleArn,
                loggingConfiguration, loggingConfiguration != null,
                tracingConfiguration, tracingConfiguration != null,
                null, false, false, null)).stateMachine();
    }
    ...
    /**
     * Resets the gap a Task's {@code HeartbeatSeconds} allows, rejecting unknown tokens.
     */
    ...
    public double redriveExecution(String executionArn) {
        ...
        if (!"STANDARD".equals(sm.getType())) {
            throw new AwsException("ExecutionNotRedrivable", "Only STANDARD executions can be redriven.", 400);
        }
        exec.setStatus("RUNNING");
        ...
        ExecutionHistory history = new ExecutionHistory(() -> executionStore.put(executionArn, exec));
        exec.setHistory(history);
        ...
        aslExecutor.executeAsync(sm, exec, history, (updatedExec, updatedHistory) -> {
            executionStore.put(updatedExec.getExecutionArn(), updatedExec);
            historyCache.put(updatedExec.getExecutionArn(), history);
        });
    ...
    public List<MapRun> listMapRuns(String executionArn) {
        ...
        return mapRunStore.scan(key -> true).stream()
