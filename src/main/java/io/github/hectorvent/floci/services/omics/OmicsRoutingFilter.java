...
/**
 * HealthOmics restJson1 paths such as {@code /sequencestore} and {@code /run}
 * collide with S3's path-style {@code /{bucket}} catch-all. Floci serves every
 * service on one port, so SigV4 credential scope {@code omics} is rewritten onto
 * an internal prefix the Omics controller owns. Tag APIs stay on
 * {@code /tags/{arn}} and are dispatched by {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class OmicsRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/omics";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isOmics(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isOmics(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && OmicsService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }
