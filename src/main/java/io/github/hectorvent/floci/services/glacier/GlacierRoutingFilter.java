...
/**
 * Amazon S3 Glacier restJson1 paths live under {@code /{accountId}/vaults/...}.
 * Floci serves every service on one port, so those paths would otherwise match
 * S3's {@code /{bucket}/{key}} catch-all. SigV4 credential scope {@code glacier}
 * (and the Glacier version header) is rewritten onto an internal prefix the
 * Glacier controller owns.
 */
@Provider
@PreMatching
@Priority(10)
public class GlacierRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-glacier";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isGlacier(
                ...
                requestContext.getHeaderString(VERSION_HEADER))) {
            return;
        }
    ...
    static boolean isGlacier(String authorization, String glacierVersion) {
        if (glacierVersion != null && !glacierVersion.isBlank()) {
            return true;
        }
