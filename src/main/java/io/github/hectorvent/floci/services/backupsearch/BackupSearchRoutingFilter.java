...
/**
 * AWS Backup Search restJson1 paths such as {@code /search-jobs/{id}/search-results}
 * collide with S3's path-style {@code /{bucket}/{key:.+}} catch-all. Floci serves every
 * service on one port, so SigV4 credential scope {@code backup-search} is rewritten onto
 * an internal prefix the Backup Search controller owns. Tag APIs stay on
 * {@code /tags/{arn}} and are dispatched by {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class BackupSearchRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-backup-search";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isBackupSearch(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isBackupSearch(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && BackupSearchService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }

    /**
     * Prefixes backup-search-signed paths onto {@link #INTERNAL_PREFIX}, stripping a trailing
     * slash so RestAssured and the SDK hit the same resource. {@code /tags} is left alone for
