...
/**
 * Glue DataBrew restJson1 paths such as {@code /datasets/{Name}} collide with S3's
 * path-style {@code /{bucket}/{key:.+}} catch-all. Floci serves every service on one
 * port, so SigV4 credential scope {@code databrew} is rewritten onto an internal
 * prefix the DataBrew controller owns. Tag APIs stay on {@code /tags/{arn}} and are
 * dispatched by {@code SharedTagsController}.
 */
@Provider
@PreMatching
@Priority(10)
public class DataBrewRoutingFilter implements ContainerRequestFilter {

    static final String INTERNAL_PREFIX = "/aws-databrew";
    ...
    public void filter(ContainerRequestContext requestContext) {
        if (!isDataBrew(requestContext.getHeaderString("Authorization"))) {
            return;
        }
    ...
    static boolean isDataBrew(String authorization) {
        if (authorization == null || authorization.isBlank()) {
            return false;
        }
        Matcher matcher = SERVICE_PATTERN.matcher(authorization);
        return matcher.find()
                && DataBrewService.SERVICE.equals(matcher.group(1).toLowerCase(Locale.ROOT));
    }

    /**
     * Prefixes databrew-signed paths onto {@link #INTERNAL_PREFIX}, stripping a trailing
