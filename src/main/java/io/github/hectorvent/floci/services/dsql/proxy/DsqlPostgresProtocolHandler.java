 ...
 * </ol>
 */
public class DsqlPostgresProtocolHandler {

    private static final Logger LOG = Logger.getLogger(DsqlPostgresProtocolHandler.class);
    ...
    public static void setServerSslContext(SSLContext context) {
        serverSslContext = context;
    }

    public static void handleAuth(Socket client, Socket backend,
                                  String masterUsername, String masterPassword, String dbName,
                                  boolean iamEnabled, DsqlSigV4Validator sigV4,
    ...
    private static SSLContext serverSslContext() throws Exception {
        ...
        synchronized (DsqlPostgresProtocolHandler.class) {
    ...
    private static SSLContext createServerSslContext() throws Exception {
        ...
        certificateBuilder.addExtension(Extension.subjectAlternativeName, false, new GeneralNames(new GeneralName[] {
                ...
                new GeneralName(GeneralName.dNSName, "*.dsql.us-east-1.on.aws"),
                new GeneralName(GeneralName.dNSName, "*.dsql.us-west-2.on.aws")
        }));
        ...
        keyStore.setKeyEntry("floci-dsql-postgres", keyPair.getPrivate(), password, new java.security.cert.Certificate[] {certificate});
    ...
    private static void bridge(Socket client, Socket backend) {
        ...
        Thread t1 = Thread.ofVirtual().name("dsql-pg-c2b")
                .start(() -> relay(clientIn, backendOut, closeBoth));
        Thread t2 = Thread.ofVirtual().name("dsql-pg-b2c")
