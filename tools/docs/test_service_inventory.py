...
# README
# --------------------------------------------------------------------------- #
def test_link_only_readme_delegates_to_local_matrix():
    assert si.readme_delegates_to_matrix("[Services](docs/services/index.md)")
    assert not si.readme_delegates_to_matrix("A README with no inventory or link")
    assert not si.readme_delegates_to_matrix("[Services](https://example.com/services/)")
...
def test_matrix_link_does_not_hide_a_present_readme_table(header):
    assert not si.readme_delegates_to_matrix("[Services](docs/services/index.md)\n" + header)
...
def test_delegated_readme_reports_no_duplicate_inventory_and_keeps_matrix_findings():
    result = si.reconcile(_data(
        readme_delegated=True, readme_categories=[], readme_details=[],
        doc_pages=["iam", "s3", "undocumented"],
    ))
    assert result["readme"]["delegated_to_matrix"] is True
    assert result["readme"]["missing_from_categories"] == []
    assert result["readme"]["missing_from_details"] == []
    assert result["docs"]["pages_without_row"] == ["undocumented"]
    assert "README links to the Service Matrix" in si.render(result)
...
def test_real_sources_parse_and_render():
    ...
    for name, count in counts.items():
        if name.startswith("readme_") and result["readme"]["delegated_to_matrix"]:
            assert count == 0
        else:
            assert count > 50
