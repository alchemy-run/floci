...
def test_route53_registry_preserves_canonical_dns_action():
    entries, _ = r._load_registry(r._repo_root())
    route53 = next(entry for entry in entries if entry.service == "route53")
    actions = r.extract_actions(route53)
    assert actions.count("TestDNSAnswer") == 1
    assert "TestDnsAnswer" not in actions
