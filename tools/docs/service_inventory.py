...
def readme_delegates_to_matrix(md_source: str) -> bool:
    """A link-only fork README deliberately has no duplicate service inventory."""
    return (
        re.search(r"\]\(docs/services/index\.md(?:#[^)]*)?\)", md_source) is not None
        and "| Category | Services |" not in md_source
        and "| Service | How it works | Notable features |" not in md_source
    )
...
def collect(repo_root: Path) -> dict:
    ...
    readme_delegated = readme_delegates_to_matrix(readme_src)
    ...
    return {
        ...
        "readme_delegated": readme_delegated,
        "readme_categories": [] if readme_delegated else parse_readme_category_names(readme_src),
        "readme_details": [] if readme_delegated else parse_readme_detail_rows(readme_src),
...
def reconcile(data: dict, today: date | None = None) -> dict:
    ...
    return {
        ...
        "readme": {
            "delegated_to_matrix": data.get("readme_delegated", False),
            "unmatched_category_names": unmatched_names,
            "missing_from_categories": [] if data.get("readme_delegated") else uncovered(category_covered, facets=False),
            "missing_facet_rows": [] if data.get("readme_delegated") else uncovered(category_covered, facets=True),
            "missing_from_details": [] if data.get("readme_delegated") else uncovered(detail_covered, facets=False),
        },
    }
...
def render(result: dict) -> str:
    ...
    if rd["delegated_to_matrix"]:
        note("README links to the Service Matrix instead of duplicating service tables.")
        note("Category and detail counts are zero by design; matrix coverage is checked above.")
    else:
