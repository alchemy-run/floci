...
| CreateRoute | Adds an IPv4, IPv6, or prefix-list route; supports gateway, NAT gateway, egress-only internet gateway, and VPC peering targets. |
...
VPC attachments, explicit route tables, associations, propagation, and static routes are
managed by the APIs described below; these remain control-plane metadata rather than packet routing.
...
A route table's `CreateRoute` with a `NetworkInterfaceId` target is accepted but not recorded, since `Route` does not yet model an ENI target; a subsequent `plan` against such a route may show drift.
