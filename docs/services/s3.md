...
| **Accelerate** | PutBucketAccelerateConfiguration, GetBucketAccelerateConfiguration |
...
**RestoreObject** rejects non-archived objects with `403 InvalidObjectState` (STANDARD / STANDARD_IA / INTELLIGENT_TIERING). Glacier-class objects are accepted (`202`) but the restore state machine is still stubbed.

**Object Lock** object APIs (`Get/PutObjectRetention`, `Get/PutObjectLegalHold`) return `400 InvalidRequest` ("Bucket is missing Object Lock Configuration") when the bucket does not have Object Lock enabled : matching live AWS.

**Presigned PUT** URLs that list `content-type` in `X-Amz-SignedHeaders` reject a mismatched `Content-Type` with `403 SignatureDoesNotMatch`, even when `floci.auth.validate-signatures` is false. Floci HMAC URLs (signed headers `host` only) are unchanged.
...
The floci-duck sidecar starts lazily on the **first non-literal Athena query**; the fork's literal SELECT fast path does not start it. Until then, `isAvailable()` returns false and S3 Select falls back to the Java evaluator for CSV and JSON. Once the sidecar is running, subsequent S3 Select calls route through DuckDB automatically.
...
`FLOCI_SERVICES_S3_ENFORCE_AUTH` controls authorization only. Cryptographic SigV4 verification of the `Authorization` header (signature, required `x-amz-content-sha256`, date and clock-skew checks, unknown access keys) and of presigned URL query signatures follows the global `FLOCI_AUTH_VALIDATE_SIGNATURES` switch, as for every other Floci service:

| `S3_ENFORCE_AUTH` | `AUTH_VALIDATE_SIGNATURES` | Behavior |
|---|---|---|
| `false` | any | No S3 authorization. Presigned URLs use Floci HMAC verification when signature validation is on. |
| `true` | `false` | Signatures are not verified. Requests are authorized (bucket policy, ACLs) as the principal named by the credential's access key ID. |
| `true` | `true` | Header and presigned SigV4 signatures are verified against the key's secret, then authorized. |

Presigned POST form uploads are verified whenever `FLOCI_SERVICES_S3_ENFORCE_AUTH` is on.
...
- copy-based metadata updates support `x-amz-metadata-directive: REPLACE` for user metadata and content type (`application/octet-stream` is stored as AWS's `binary/octet-stream` default), but do not yet cover every AWS copy header
