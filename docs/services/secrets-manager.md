...
| `RotateSecret` | Trigger secret rotation via a Lambda or the owning service. Lambda rotation does not pre-create an empty `AWSPENDING` version : the rotation function's `PutSecretValue` creates it, matching AWS so templates that skip the put when `ListSecretVersionIds` already lists the token still write a value. |
| `CancelRotateSecret` | Turn off automatic rotation, retain the Lambda ARN and rules, and report any half-staged `AWSPENDING` version |
