...
- The add-on snapshot includes the upstream add-on catalog and preserves the historical `vpc-cni` version
...
!!! warning "Catalog presence is not installation"
    Catalog reads do not create installations. `CreateAddon` and `UpdateAddon` record
    managed add-on metadata, but do not install Kubernetes workloads. `ListAddons`
    returns only explicitly created records, not k3s-bundled components. These
    operations validate the cluster in the requesting account and region.
...
When enabled (`FLOCI_SERVICES_EKS_IMDS=true`), each k3s cluster container exposes the AWS Instance Metadata Service on the link-local address `169.254.169.254:80`. Inside the container, Floci adds `169.254.169.254/32` to the loopback interface (`lo`) and runs an authenticated Python HTTP proxy forwarding metadata requests to Floci's IMDS server. The proxy capability is stored in a root-only `0600` file, never passed on the command line, and replaces caller-supplied capability headers. Direct unauthenticated requests to the backing IMDS server are rejected.

Both IMDSv1 and IMDSv2 (`PUT /latest/api/token`) are supported. The cluster container is registered as a synthesized EC2 instance node (type `m5.large`, image `ami-eks-k3s`) associated with a distinct synthesized node instance profile, not the control-plane IAM role. Stopping the cluster revokes its proxy identity and metadata tokens:
...
EC2 metadata options store `HttpPutResponseHopLimit`, but the userspace HTTP proxy terminates the incoming connection and opens a new connection to Floci, regenerating the IP packet TTL. Hop limits are recorded on the instance metadata options model but are not enforced by the userspace proxy.
